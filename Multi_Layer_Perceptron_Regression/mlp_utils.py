...
def create_target(train_m, test_m):
    ...
    dummies_train = pd.get_dummies(train_m['Kantons-Nummer'], prefix='canton',
                                   drop_first=True,
                                   dtype=int)
    dummies_test = pd.get_dummies(test_m['Kantons-Nummer'], prefix='canton',
                                  drop_first=True,
                                  dtype=int)
    ...
    train_m = pd.concat([train_m, dummies_train], axis=1)
    test_m = pd.concat([test_m, dummies_test], axis=1)

    train_m = train_m.drop(columns=['Kantons-Nummer'])
    test_m = test_m.drop(columns=['Kantons-Nummer'])

    return y_train, train_m, test_m
